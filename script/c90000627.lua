...
function c90000627.initial_effect(c)
	...
	e3:SetCondition(c90000627.descon)
	e3:SetTarget(c90000627.destg)
	e3:SetOperation(c90000627.desop)
	c:RegisterEffect(e3)
end
...
--ToPendulum
function c90000627.descon(e,tp,eg,ep,ev,re,r,rp)
...
function c90000627.desfilter(c)
	return c:IsType(TYPE_PENDULUM) and c:IsDestructable()
end
function c90000627.destg(e,tp,eg,ep,ev,re,r,rp,chk)
	local c=e:GetHandler()
	if chk==0 then return Duel.IsExistingMatchingCard(c90000627.desfilter,tp,LOCATION_SZONE,0,1,c) end
	local sg=Duel.GetMatchingGroup(c90000627.desfilter,tp,LOCATION_SZONE,0,c)
	Duel.SetOperationInfo(0,CATEGORY_DESTROY,sg,sg:GetCount(),0,0)
end
function c90000627.desop(e,tp,eg,ep,ev,re,r,rp)
	local sg=Duel.GetMatchingGroup(c90000627.desfilter,tp,LOCATION_SZONE,0,e:GetHandler())
