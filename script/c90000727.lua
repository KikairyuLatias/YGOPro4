...
function c90000727.initial_effect(c)
	...
	e6:SetCountLimit(1)
	e6:SetTarget(c90000727.target)
	e6:SetOperation(c90000727.activate)
	c:RegisterEffect(e6)
end
--Bunny Pendulum SS
...
function c90000727.cost(e,tp,eg,ep,ev,re,r,rp,chk)
	if chk==0 then return Duel.GetCustomActivityCount(90000727,tp,ACTIVITY_SPSUMMON)==0 end
...
function c90000727.filter(c)
	return c:IsSetCard(0x7d0) and (c:IsLocation(LOCATION_GRAVE) or c:IsFaceup()) and c:IsAbleToDeck()
end
function c90000727.target(e,tp,eg,ep,ev,re,r,rp,chk)
	if chk==0 then return Duel.IsPlayerCanDraw(tp,2)
		and Duel.IsExistingMatchingCard(c90000727.filter,tp,LOCATION_EXTRA+LOCATION_GRAVE,0,3,nil) end
	Duel.SetOperationInfo(0,CATEGORY_TODECK,nil,3,0,0)
	Duel.SetOperationInfo(0,CATEGORY_DRAW,nil,0,tp,2)
end
function c90000727.activate(e,tp,eg,ep,ev,re,r,rp)
	local g=Duel.GetMatchingGroup(aux.NecroValleyFilter(c90000727.filter),tp,LOCATION_EXTRA+LOCATION_GRAVE,0,nil)
