...
function c90001232.initial_effect(c)
	--pendulum summon
	...
	e3:SetCountLimit(1)
	e3:SetTarget(c90001232.target)
	e3:SetOperation(c90001232.activate)
	c:RegisterEffect(e3)
	...
	e4:SetCountLimit(1)
	e4:SetTarget(c90001232.target2)
	e4:SetOperation(c90001232.operation2)
	c:RegisterEffect(e4)
	--go to P-zone
	...
	e5:SetCondition(c90001232.pencon)
	e5:SetTarget(c90001232.pentg)
	e5:SetOperation(c90001232.penop)
	c:RegisterEffect(e5)
	--Flyer power up
		--atk up
		...
		e6:SetTarget(c90001232.tg)
		e6:SetValue(c90001232.val)
		c:RegisterEffect(e6)
		--def up
		...
		e7:SetValue(c90001232.val)
		e7:SetTarget(c90001232.tg)
		c:RegisterEffect(e7)
	--Skill Drain
	local e8=Effect.CreateEffect(c)
	e8:SetType(EFFECT_TYPE_FIELD)
	e8:SetRange(LOCATION_MZONE)
	e8:SetTargetRange(0,LOCATION_PZONE)
...
--immunity
function c90001232.indval(e,re,tp)
	return tp~=e:GetHandlerPlayer()
end

--special summon
function c90001232.filter(c)
	return c:IsType(TYPE_MONSTER) and c:IsSetCard(0x4c9) or c:IsCode(90001215) 
end
function c90001232.target(e,tp,eg,ep,ev,re,r,rp,chk)
	if chk==0 then return Duel.GetLocationCount(tp,LOCATION_MZONE)>0
		and Duel.IsExistingMatchingCard(c90001232.filter,tp,LOCATION_HAND+LOCATION_DECK+LOCATION_GRAVE,0,1,nil,e,tp) end
	Duel.SetOperationInfo(0,CATEGORY_SPECIAL_SUMMON,nil,1,tp,LOCATION_HAND+LOCATION_DECK+LOCATION_GRAVE)
end
function c90001232.activate(e,tp,eg,ep,ev,re,r,rp)
	if Duel.GetLocationCount(tp,LOCATION_MZONE)<=0 then return end
	Duel.Hint(HINT_SELECTMSG,tp,HINTMSG_SPSUMMON)
	local g=Duel.SelectMatchingCard(tp,c90001232.filter,tp,LOCATION_HAND+LOCATION_DECK+LOCATION_GRAVE,0,1,1,nil,e,tp)
...
-- lock and fire
function c90001232.target2(e,tp,eg,ep,ev,re,r,rp,chk)
...
function c90001232.operation2(e,tp,eg,ep,ev,re,r,rp)
	Duel.Hint(HINT_SELECTMSG,tp,HINTMSG_REMOVE)
	local tc=Duel.SelectMatchingCard(tp,Card.IsAbleToRemove,tp,0,LOCATION_ONFIELD+LOCATION_HAND,1,1,nil)
	e:GetHandler():RegisterFlagEffect(90001232,RESET_EVENT+0x1fe0000,0,1)
...
--boost
function c90001232.tg(e,c)
	return c:IsSetCard(0x4c9) and c:IsType(TYPE_MONSTER)
end
function c90001232.filter2(c)
