...
function c90000143.initial_effect(c)
	--pendulum summon
	...
	e1:SetDescription(aux.Stringid(90000143,0))
	e1:SetCategory(CATEGORY_SPECIAL_SUMMON)
	e1:SetType(EFFECT_TYPE_SINGLE+EFFECT_TYPE_TRIGGER_O)
	e1:SetCode(EVENT_SUMMON_SUCCESS)
	e1:SetProperty(EFFECT_FLAG_DAMAGE_STEP+EFFECT_FLAG_DELAY)
	e1:SetTarget(c90000143.sptg)
	e1:SetOperation(c90000143.spop)
	c:RegisterEffect(e1)
	local e2=e1:Clone()
	...
	e4:SetDescription(aux.Stringid(90000143,1))
	e4:SetCategory(CATEGORY_TODECK+CATEGORY_DRAW)
	e4:SetProperty(EFFECT_FLAG_CARD_TARGET)
	e4:SetType(EFFECT_TYPE_IGNITION)
	e4:SetRange(LOCATION_MZONE)
	e4:SetCountLimit(1)
	e4:SetTarget(c90000143.drtg)
	e4:SetOperation(c90000143.drop)
	c:RegisterEffect(e4)
	--power up
		...
		e5:SetTarget(c90000143.tg)
		e5:SetValue(c90000143.val)
		c:RegisterEffect(e5)
		--def up
		local e6=e5:Clone()
...
function c90000143.sptg(e,tp,eg,ep,ev,re,r,rp,chk)
	if chk==0 then return Duel.GetLocationCount(tp,LOCATION_MZONE)>0
		and Duel.IsExistingMatchingCard(c90000143.filter,tp,LOCATION_DECK+LOCATION_REMOVED,0,1,nil,e,tp) end
	Duel.SetOperationInfo(0,CATEGORY_SPECIAL_SUMMON,nil,1,tp,LOCATION_DECK+LOCATION_REMOVED)
end
function c90000143.spop(e,tp,eg,ep,ev,re,r,rp)
	if Duel.GetLocationCount(tp,LOCATION_MZONE)<=0 then return end
	Duel.Hint(HINT_SELECTMSG,tp,HINTMSG_SPSUMMON)
	local g=Duel.SelectMatchingCard(tp,c90000143.filter,tp,LOCATION_DECK+LOCATION_REMOVED,0,1,1,nil,e,tp)
	if g:GetCount()>0 then
		Duel.SpecialSummon(g,0,tp,tp,false,false,POS_FACEUP)
	end
end
--recycle draw
function c90000143.tdfilter(c)
...
function c90000143.drop(e,tp,eg,ep,ev,re,r,rp)
...
--boost
function c90000143.tg(e,c)
	return c:IsSetCard(0x5f3) and c:IsType(TYPE_MONSTER)
end
function c90000143.filter2(c)
	return c:IsFaceup() and c:IsSetCard(0x5f3)
end
function c90000143.val(e,c)
	return Duel.GetMatchingGroupCount(c90000143.filter2,c:GetControler(),LOCATION_MZONE,0,nil)*200
end